        ...
        <el-row>
            <el-col :span="24">
                <el-card shadow="never">
                    <div slot="header">
                        <span>{{$route.params.id ? 'Edit' : 'Create'}} Document</span>
                    </div>
                    <el-form ref="form" @submit.native.prevent="onSubmit" :model="form" label-width="120px">
                        <el-form-item prop="products"  label="products">
                            <el-select
...
<script>
    import search_product from './../product/search_product'
    export default {
        mixins:[search_product],
        data() {
            return {
                form: {
                    product_id: '',
        ...
        beforeRouteEnter (to, from, next) {
            if (to.params.id) {
                axios.get(`/api/documents/${to.params.id}`).then(function (response) {
        ...
        methods: {
            ...
            setData(response){
                var vm = this
                vm.products = []
                vm.products = response.products.map(item => {
            ...
            onCancel(){
                this.$router.push({name: 'view-document'})
            },
            onSubmit() {
                var vm = this
                this.$refs.form.validate((valid) => {
                    if (valid) {
                        ...
                        axios[id ? 'put' : 'post'](`/api/documents${id ? `/${id}` : ''}`, vm.form).then(function () {
                            vm.disabled = false
                            vm.$message({
                                type: 'success',
